...
output "ecr_repository_url" {
  value = aws_ecr_repository.rag_app_ecr.repository_url
  description = "ECR 仓库完整 URL"
}
output "apprunner_service_arn" {
  value = can(aws_apprunner_service.rag_app_service[0].arn) ? aws_apprunner_service.rag_app_service[0].arn : null
  description = "复制到 GitHub Secret: APP_RUNNER_ARN (如果通过 GitHub Actions 部署，此值可能为 null，需要手动创建服务后获取 ARN)"
}
output "apprunner_service_name" {
  value = "bee-edu-rag-service"
  description = "App Runner 服务名称（用于 GitHub Actions 部署）"
}
output "apprunner_access_role_arn" {
  value = aws_iam_role.apprunner_service_role.arn
  description = "App Runner Access Role ARN (用于从 ECR 拉取镜像)"
}
output "apprunner_instance_role_arn" {
  value = aws_iam_role.apprunner_instance_role.arn
  description = "App Runner Instance Role ARN (用于运行时访问 Secrets Manager)"
}
output "openai_secret_arn" {
  value = aws_secretsmanager_secret.openai_key.arn
  description = "OpenAI API Key 在 Secrets Manager 中的 ARN (用于 GitHub Actions 部署配置)"
}
